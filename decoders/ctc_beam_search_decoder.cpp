...
std::vector<BeamResults> ctc_beam_search_decoder_ex(
  ...
  for (size_t time_step = 0; time_step < num_time_steps; ++time_step) {
    ...
    for (size_t index = 0; index < log_prob_idx.size(); index++) {
      ...
      for (size_t i = 0; i < prefixes.size() && i < beam_size; ++i) {
        ...
        if (prefix_new != nullptr) {
          float log_p = -NUM_FLT_INF;
		prefix_new->offset = timestep_offset + time_step;
  ...
  if (ext_scorer != nullptr && !ext_scorer->is_character_based()) {
    for (size_t i = 0; i < beam_size && i < prefixes.size(); ++i) {
      ...
      if (!prefix->is_empty() && prefix->character != space_id) {
        ...
        score += ext_scorer->beta;
	   //prefix->score += score;
        prefix->beam_score = prefix->score + score;
	   //printf("set beam score of beam %d to %f (score = %f)\n", i, prefix->beam_score, prefix->score);
      }
    }
  }
  else
  {
    for (size_t i = 0; i < beam_size && i < prefixes.size(); ++i)	  
      prefixes[i]->beam_score = prefixes[i]->score;
  }
  
  size_t num_prefixes = std::min(prefixes.size(), beam_size);
  std::sort(prefixes.begin(), prefixes.begin() + num_prefixes, prefix_compare_beam);
  
  if( prefixes.size() > top_k )
	  prefixes.erase(prefixes.begin() + top_k, prefixes.end());
  
  // compute aproximate ctc score as the return score, without affecting the
  // return order of decoding result. To delete when decoder gets stable.
  /* dfranklin - disabled this out as approx_ctc is not used/returned
  for (size_t i = 0; i < beam_size && i < prefixes.size(); ++i) {
    double approx_ctc = prefixes[i]->score;
    if (ext_scorer != nullptr) {
      std::vector<int> output;
  ...
  std::vector<BeamResults> output_vecs;
  ...
  for (size_t i = 0; i < beam_size && i < prefixes.size(); ++i) {
    ...
    std::string beam_str;
    for (size_t j = 0; j < char_indices.size(); j++) {
      beam_str += vocabulary[char_indices[j]];
    }
    
    BeamResults beam_results;
    
    beam_results.text = beam_str;
    beam_results.score = prefixes[i]->beam_score;
    
    // tokenize words
    int token_start = -1;
    
    for( size_t n=0; n < beam_str.size(); n++ )
    {
	    const bool is_last = (n == (beam_str.size() - 1));
	    
	    if( beam_str[n] == ' ' || is_last )
	    {
		    if( token_start < 0 )
			    continue;
		    ...
		    word.text = beam_str.substr(token_start, n - token_start + (is_last ? 1 : 0));
		    word.start_time = timestamps[token_start];
		    word.end_time = timestamps[n - (is_last ? 0 : 1)]; // [n- 1];
		    
		    // sum the log propabilities of the word
		    double exp_sum = std::exp(scores[token_start]); // TODO subtract max
		    
		    for( size_t k=token_start + 1; k < n; k++ )
		    {
			    //printf("beam %d char %d %d  adding %g to %g\n", i, n, k, exp_sum, std::exp(scores[k]));
			    exp_sum += std::exp(scores[k]);
		    }
		    
		    word.score = std::log(exp_sum); 
		    
		    beam_results.words.push_back(word);
		    token_start = -1;
	    }
    ...
    output_vecs.emplace_back(beam_results);
  }

  // update word list with word and corresponding start & end times
  /*
  if (output_vecs[0].text.size() > 0) {
    int ts_idx = 0;
    char* saveptr;
    char transcript[output_vecs[0].text.size() + 1];
    strcpy(transcript, output_vecs[0].text.size());
...
BeamDecoder::get_word_timestamps_buffer()
{
  std::vector<std::pair<std::string, std::pair<int, int>>> words;
  
  // TODO prev words
  for( int i=0; i < wordlist.size(); i++ )
	  words.push_back( std::pair<std::string, std::pair<int, int>>( std::get<0>(wordlist[i]),
				    std::pair<int, int>(std::get<1>(wordlist[i]), std::get<2>(wordlist[i])) ) );
